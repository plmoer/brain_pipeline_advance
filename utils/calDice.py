...
def cal_dice(pred, target, bGPU=False, nBatch_size=1):
    ...
    target_label = maskConversionObj(target_arr)
    pred_label = maskConversionObj(pred_arr)
    dice = diceCompute(pred_label, target_label)
    return dice
